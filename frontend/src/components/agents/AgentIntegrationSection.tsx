...
import { Terminal, Cloud, Ship, Copy, Check, Server, ShieldCheck } from "lucide-react";
import { AttestationPolicyCard } from "./AttestationPolicyCard";

type RuntimeTab = "env" | "gcp" | "aws" | "k8s" | "attestation";
...
const TABS: TabConfig[] = [
  ...
  },
  {
    ...
    label: "GCP",
    icon: Server,
    description: "Use Workload Identity for zero-secret authentication on GCP",
  },
  {
    key: "aws",
    label: "AWS",
  ...
  },
  {
    ...
    icon: ShieldCheck,
    description: "View platform attestation policies for this agent",
  },
];
...
function gcpSnippet(agentId: string): string {
  return `# 1. Bind a GCP Service Account to your Cloud Run / GKE workload.
# 2. Register the agent with the service account email as the selector:
#
#   deepsecure agent register \\
#     --agent-id "${agentId}" \\
#     --platform gcp_workload_identity \\
#     --selector "my-agent@my-project.iam.gserviceaccount.com"
#
# 3. In your agent code — no static keys needed:

from deepsecure import Client

client = Client()  # auto-detects GCP Workload Identity
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        {activeTab === "attestation" ? (
          <AttestationPolicyCard agentId={agentId} />
        ) : (
